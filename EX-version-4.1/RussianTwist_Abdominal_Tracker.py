...
def Rtwist():
    ...
    print("Running Russian_Twist.py")

    # "Resources/man_performing_bicep_curls_micro.mp4"
    cap = cv2.VideoCapture("Resources/russian_twist_abdominal.mp4")
    ...
    error_report[0][0] = 'ERROR: Your arm isn\'t straight enough'
    '''
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            angle_12_c = detector.findAngle(img, 11, 12, 24, 0)
            ...
            #angle_23_a = detector.findAngle(img, 11, 23, 25, 1) #you might also need this
            angle_23_b = detector.findAngle(img, 24, 23, 25, 0)

            # angle_24_a = detector.findAngle(img, 12, 24, 26, 1)
            angle_24_b = detector.findAngle(img, 23, 24, 26, 0)
            ...
            #perc_angle_b = np.interp( angle_23_b , (), (0, 100) )
            #print(perc_angle_b)
            ...
            if count < max_count:
                if   (   (angle_23_b < 132 and angle_23_b > 123) and (angle_24_b < 310 and angle_24_b > 290)  ):
                    cv2.putText(img, "Initial State", (10, 25), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)
                    rep_count_locked = 0

                elif (   (angle_23_b < 172 and angle_23_b > 153) and (angle_24_b < 360 and angle_24_b > 340) and rep_count_locked == 0   ):
                ...
                elif (  (angle_23_b < 80 and angle_23_b > 65)  and (angle_24_b < 263 and angle_24_b > 233) and rep_count_locked == 0 ):
            ...
            if (rep_count_locked == 0 and count < max_count):
                ...
                if ( (angle_14_a < 180) or (angle_14_a > 270) ):
                    ...
                    error_keeper[round(count)] = 0
                    cv2.putText(img, "ERROR: Your arm isn't straight enough", (10, txt_w), cv2.FONT_HERSHEY_PLAIN, 1,
                                (255, 0, 0), )
    ...
    workbook = xlsxwriter.Workbook('Exercise_Reports/RussianTwist_error_report.xlsx')  # Change file name acc. to exercise
